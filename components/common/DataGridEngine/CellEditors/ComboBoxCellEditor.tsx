...
export class ComboBoxCellEditor extends React.Component<any, any> {
    ...
    renderOptions() {
        ...
        if (this.state.options) {
            for (let index in this.state.options) {
                ...
                retorno.push(<option value={option[this.props.props.dataSource.valueKeyField]}
                                     selected={selected}>{texto_opcion}</option>);
            }
        }
        return retorno;
    }
    ...
    render() {
        return (
            <select className={"SelectField"} value={this.state.value} onChange={this.handleChange.bind(this)}
                    ref={(input) => {
                        this.input = input;
                    }}>
                {this.renderOptions()}
